...
export default function CategoriesClient({ initial }: { initial: C[] }) {
  ...
  const addNew = () => {
    ...
    // Scroll to and focus the new row's name field so it's obvious something happened.
  ...
  const save = async (c: C, i: number) => {
    ...
    const j = await r.json();
    toast.success("Saved");
    // Reflect the (possibly auto-uniquified) id/slug returned by the server
    // so the row shown in the UI always matches what's actually in the database.
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={addNew} className="bg-navy text-white rounded px-4 py-2.5 text-sm flex items-center gap-2"><Plus className="w-4 h-4" /> Add</button>
      </div>
